...
func main() {
	// part 1
	//graph := createGraph()
	//inverseGraph := invert(graph)
	//visited := make(map[string]bool)
	//traverse(inverseGraph["shiny gold"], inverseGraph, visited)
	//fmt.Println(len(visited))

	graph := createGraph()
	sum := traverseCount(graph["shiny gold"], graph) - 1
	fmt.Println(sum)
}

func traverseCount(options []Edge, graph map[string][]Edge) int {

	// gold -> 1 olive, 2 plum
	// olive -> 3 blue, 4 black (7)
	// blue -> 0
	// black -> 0
	// plum -> 5 blue, 6 black

	// total = 1 gold + 1 olive * (3 blue * 1 + 4 black * 1) + 2 plum * (5 blue * 1 + 6 black * 1)
	// = 1 + 1 * 7 + 2*(5 + 6) = 1 + 7 + 22 = 32
	if len(options) == 0 {
		return 1
	} else {
		subtotal := 1
		for i := range options {
			edge := options[i]
			factor := edge.cost
			lowerLevelSum := traverseCount(graph[edge.id], graph)
			subtotal += factor * lowerLevelSum
		}
		return subtotal
	}
}

func traverse(options []string, graph map[string][]string, visited map[string]bool) {
	for i := range options {
		option := options[i]
		if !visited[option] {
			traverse(graph[option], graph, visited)
		}
		visited[option] = true
	}
}

func invert(graph map[string][]string) map[string][]string {
	// a -> [b,c]
	// b -> [c]
	...
	for key, values := range graph {
		for i := range values {
			value := values[i]
			inverseGraph[value] = append(inverseGraph[value], key)
		}
	}
...
func createGraph() map[string][]Edge {
	//	input := `
...
bright lavender bags contain 4 striped blue bags.
	...
	for i := range lines {
		s := regexp.MustCompile(`bags contain`).Split(strings.TrimSpace(lines[i]), -1)
		colorId := strings.TrimSpace(s[0])
		colorDesc := s[1]
		s = regexp.MustCompile(`bags|bag`).Split(strings.TrimSpace(colorDesc), -1)
		emptyPattern := regexp.MustCompile(`.*no other.*`)
		colorPattern := regexp.MustCompile(`[^a-z]*([0-9]+)\s+([a-z]+\s+[a-z]+)[^a-z]*`)
		for index := range s {
			if emptyPattern.MatchString(s[index]) {
				graph[colorId] = []Edge{}
			} else if colorPattern.MatchString(s[index]) {
				colorGroup := colorPattern.FindStringSubmatch(s[index])
				cost, _ := strconv.Atoi(colorGroup[1])
				color := strings.TrimSpace(colorGroup[2])
				graph[colorId] = append(graph[colorId], Edge{
					cost: cost,
					id:   color,
				})
			}
