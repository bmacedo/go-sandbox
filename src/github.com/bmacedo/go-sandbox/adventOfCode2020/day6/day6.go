...
sbzulxv
	...
	for i := range group {
		...
		// create set of possible answers
		...
		// foreach possible answer, check if everyone answered yes
		personalAnswers := regexp.MustCompile(`\n`).Split(strings.TrimSpace(answer), -1)
		for key := range set {
			for j := range personalAnswers {
				p := personalAnswers[j]
				if !strings.Contains(p, string(key)) {
					delete(set, key)
					break
				}
			}
