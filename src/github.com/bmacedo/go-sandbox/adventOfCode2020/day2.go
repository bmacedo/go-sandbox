...
In the below example, 2 passwords are valid. The middle password, cdefg, is not; it contains no
...
func main() {
	...
	count := 0
	for i := range lines {
		min, max, letter, password := parseLine(lines[i])
...
func isValid(min int, max int, letter byte, password string) bool {
	...
	length := len(password)
	return ((min-1 >= 0 && password[min-1] == letter) && (length < max-1 || password[max-1] != letter)) ||
		((min-1 < 0 || password[min-1] != letter) && (max-1 < length && password[max-1] == letter))
}

func parseLine(s string) (int, int, byte, string) {
	re := regexp.MustCompile("([0-9]+)-([0-9]+)\\s+([a-zA-Z]):\\s+([a-zA-Z]+)")
