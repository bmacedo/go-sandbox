...
func isValid(passport Passport) bool {
	hasRequiredFields := passport.byr != "" &&
		passport.iyr != "" &&
		passport.eyr != "" &&
		passport.hgt != "" &&
		passport.hcm != "" &&
	...
	hasValidByr := byr >= 1920 && byr <= 2002

	iyr, _ := strconv.Atoi(passport.iyr)
	hasValidIyr := iyr >= 2010 && iyr <= 2020

	eyr, _ := strconv.Atoi(passport.eyr)
	hasValidEyr := eyr >= 2020 && eyr <= 2030

	height, _ := strconv.Atoi(passport.hgt)
	isCm := passport.hcm == "cm"
	hasValidHeight := (isCm && height >= 150 && height <= 193) || (!isCm && height >= 59 && height <= 76)

	return hasRequiredFields && hasValidByr && hasValidIyr && hasValidEyr && hasValidHeight
}
...
`
	...
	passportStr := regexp.MustCompile(`\n\s*\n`).Split(strings.TrimSpace(input), -1)

	var passports []Passport
	for i := range passportStr {
		currentPassport := Passport{}
		addFields(passportStr[i], &currentPassport)
...
func addFields(s string, passport *Passport) {
	passport.byr = parseField(s, "byr:([0-9]{4})(\\s|$)")
	passport.iyr = parseField(s, "iyr:([0-9]{4})(\\s|$)")
	passport.eyr = parseField(s, "eyr:([0-9]{4})(\\s|$)")
	passport.hgt = parseField(s, "hgt:([0-9]+)(cm|in)(\\s|$)")
	passport.hcm = parseField(s, "hgt:[0-9]+(cm|in)(\\s|$)")
	passport.hcl = parseField(s, "hcl:(#[0-9a-f]{6})(\\s|$)")
	passport.ecl = parseField(s, "ecl:(amb|blu|brn|gry|grn|hzl|oth)(\\s|$)")
	passport.pid = parseField(s, "pid:([0-9]{9})(\\s|$)")
	passport.cid = parseField(s, "cid:(\\S+)(\\s|$)")
	//passport.byr = parseField(s, "byr:(\\S+)")
	//passport.iyr = parseField(s, "iyr:(\\S+)")
	//passport.eyr = parseField(s, "eyr:(\\S+)")
	//passport.hgt = parseField(s, "hgt:(\\S+)")
	//passport.hcl = parseField(s, "hcl:(\\S+)")
	//passport.ecl = parseField(s, "ecl:(\\S+)")
	//passport.pid = parseField(s, "pid:(\\S+)")
	//passport.cid = parseField(s, "cid:(\\S+)")
}

func parseField(s string, pattern string) string {
	rg := regexp.MustCompile(pattern)
	if rg.MatchString(s) {
		groups := rg.FindStringSubmatch(s)
		return groups[1]
	}
	return ""
}
